...
"""
def author():                                                                                             
        return 'xhang7' # replace sroutray3 with your Georgia Tech username.                                                                                             
                                                                                              
def gtid():                                                                                               
    return 903546087 # replace with your GT ID number      
...
#############################################
                if source not in self.node_degree:
                        ...
                        self.node_degree[source] += 1
                if max(source,target) > self.max_node_id:
...
### Implement your code here
#############################################
            temp_scores=(1 - damping_factor)* node_weights
            for source, target in self.read_edge_file(self.edge_file):

                 temp_scores[target]= temp_scores[target] + scores[source]/self.node_degree[source]*damping_factor
